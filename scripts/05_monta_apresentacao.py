...
function escalaVeredito(recortes){
  const V = D.placar.veredito;
  const W=980, labW=232, barIni=252, barFim=690, colV=712, rowH=74, topo=26;
  const meio=(barIni+barFim)/2, meiaLarg=(barFim-barIni)/2, ESC=0.35;
  const linhas = recortes.map(t=>({titulo:t,
      va: V.find(v=>v.titulo===t && v.leitura==='variação'),
      ni: V.find(v=>v.titulo===t && v.leitura==='nível')}))
    .filter(l=>l.va).sort((a,b)=>a.va.pct-b.va.pct);   // do melhor para o pior
  const H = topo + rowH*linhas.length;
  let g='<svg viewBox="0 0 '+W+' '+H+'" role="img" aria-label="veredito por recorte">';
  g += '<text x="'+(meio-14)+'" y="14" fill="#5E7480" font-size="11" font-family="IBM Plex Sans" text-anchor="end">pior que os pares</text>';
  g += '<text x="'+(meio+14)+'" y="14" fill="#5E7480" font-size="11" font-family="IBM Plex Sans">melhor que os pares</text>';
  g += '<line x1="'+meio+'" x2="'+meio+'" y1="20" y2="'+(H-10)+'" stroke="#5E7480" stroke-width="1.5"/>';
  linhas.forEach((l,i)=>{
    const y = topo + i*rowH + 18, bh = 26;
    const dev = 0.5 - l.va.pct;                     // positivo = melhor que a mediana
    const larg = Math.min(Math.abs(dev)/ESC, 1) * meiaLarg;
